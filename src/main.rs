...
use web3::{
    ...
    helpers as w3h,
    types::{BlockId, BlockNumber, TransactionId, H160, U256, U64},
};
...
async fn main() {
    ...
    for transaction_hash in latest_block.transactions {
        ...
        println!(
            "[{}]\n ({} -> {}) \nFrom: {}, \nTo: {}, \nValue: {}, \nGas: {}, \nGas price: {:?}\n\n",
            tx.transaction_index.unwrap_or_else(|| U64::from(0)),
            &token_name,
            &func_signature,
            ...
            eth_value,
            wei_to_eth(tx.gas),
            wei_to_eth(tx.gas_price.unwrap())
        )
    }
}

fn wei_to_eth(wei_value: U256) -> f64 {
    let res = wei_value.as_u128() as f64;
